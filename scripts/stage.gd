...
static func from_dict(params: Dictionary) -> StageData:
	...
	data.intermission = load(params.get("intermission", "uid://ct1qkka6lngc5"));
...
static var stage_variants: Dictionary[String, Dictionary] = {
	"tomato fields": {
		...
		"checkpoint": "uid://d28lqabm420p3", # same as "terrain fill"
		"terrain scale": Vector2(1e-3, 1e2), # should be a Vector2
		"stage length": 3e4, # should be a float
		"intermission": "uid://ct1qkka6lngc5", # can be a relative path to the texture, but a string nevertheless
	}
}
